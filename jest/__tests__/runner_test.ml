...
let () =
  ...
  Skip.test "test - expect fail" (fun () -> false);

  testAsync "testAsync" (fun finish -> finish true);

  Skip.testAsync "testAsync - no done" (fun _ -> ());

  Skip.testAsync "testAsync - expect fail" (fun finish -> finish false);

  testAsync "testAsync - timeout ok" ~timeout:1 (fun finish -> finish true);

  Skip.testAsync "testAsync - timeout fail" ~timeout:1 (fun _ -> ());

  testPromise "testPromise" (fun () -> Js.Promise.resolve true);
  ...
  testAll "testAll - tuples"
    [ ("foo", 3); ("barbaz", 6); ("bananas!", 8) ]
    (fun (input, output) -> Js.String.length input == output);

  describe "describe" (fun () -> test "some aspect" (fun () -> true));
  ...
  describe "beforeAllAsync" (fun () ->
      ...
      describe "with 100ms timeout" (fun () ->
          ...
          beforeAllAsync ~timeout:100 (fun finish ->
              x := !x + 4;
              finish ());
      ...
      Skip.describe "timeout should fail suite" (fun () ->
          ...
          test "" (fun () -> true)
          (* runner will crash if there's no tests *)));
  ...
  describe "beforeAllPromise" (fun () ->
      ...
      describe "with 100ms timeout" (fun () ->
          ...
          beforeAllPromise ~timeout:100 (fun () ->
              x := !x + 4;
              Js.Promise.resolve ());
  ...
  describe "beforeEachAsync" (fun () ->
      ...
      describe "with 100ms timeout" (fun () ->
          ...
          beforeEachAsync ~timeout:100 (fun finish ->
              x := !x + 4;
              finish ());
  ...
  describe "beforeEachPromise" (fun () ->
      ...
      describe "with 100ms timeout" (fun () ->
          ...
          beforeEachPromise ~timeout:100 (fun () ->
              x := !x + 4;
              Js.Promise.resolve true);
      ...
      Skip.describe "timeout should fail suite" (fun () ->
          beforeEachPromise ~timeout:1 (fun () ->
              Js.Promise.make (fun ~resolve:_ ~reject:_ -> ()));
  ...
  describe "afterAll" (fun () ->
      ...
      describe "phase 2" (fun () -> test "x is suddenly 4" (fun () -> !x == 4)));
  ...
  describe "afterAllAsync" (fun () ->
      ...
      describe "with 100ms timeout" (fun () ->
          ...
          describe "phase 1" (fun () ->
              afterAllAsync ~timeout:100 (fun finish ->
                  x := !x + 4;
                  finish ());
  ...
  describe "afterAllPromise" (fun () ->
      ...
      describe "with 100ms timeout" (fun () ->
          ...
          describe "phase 1" (fun () ->
              afterAllPromise ~timeout:100 (fun () ->
                  x := !x + 4;
                  Js.Promise.resolve true);
  ...
  describe "afterEachAsync" (fun () ->
      ...
      describe "with 100ms timeout" (fun () ->
          ...
          afterEachAsync ~timeout:100 (fun finish ->
              x := !x + 4;
              finish ());
  ...
  describe "afterEachPromise" (fun () ->
      ...
      describe "with 100ms timeout" (fun () ->
          ...
          afterEachPromise ~timeout:100 (fun () ->
              x := !x + 4;
              Js.Promise.resolve true);
      ...
      Skip.describe "timeout should fail suite" (fun () ->
          afterEachPromise ~timeout:1 (fun () ->
              Js.Promise.make (fun ~resolve:_ ~reject:_ -> ()));
  ...
  describe "Skip" (fun () ->
      ...
      Skip.testAsync "Skip.testAsync" (fun finish -> finish false);
      Skip.testAsync "Skip.testAsync - timeout" ~timeout:1 (fun _ -> ());

      Skip.testPromise "Skip.testPromise" (fun () -> Js.Promise.resolve false);
      Skip.testPromise "testPromise - timeout" ~timeout:1 (fun () ->
          Js.Promise.make (fun ~resolve ~reject:_ -> (resolve false [@u])));

      Skip.testAll "testAll" [ "foo"; "bar"; "baz" ] (fun _ -> false);
      Skip.testAll "testAll - tuples"
        [ ("foo", 3); ("barbaz", 6); ("bananas!", 8) ]
        (fun (_, _) -> false);
