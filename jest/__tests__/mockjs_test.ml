...
external bindThis : (('a -> 'b)[@u]) -> 'c -> ('a -> 'b[@u]) = "bind"
[@@mel.send]
...
external call2 : (('a -> 'b -> 'c)[@u]) -> (_[@mel.as 0]) -> 'a -> 'b -> 'c
  = "call"
[@@mel.send]
...
let _ =
  describe "inferred_fn" (fun _ ->
      ...
      test "mockReset - resets implementations" (fun _ ->
          ...
          expect (before, after)
          |> toEqual (Js.Undefined.return 128, Js.Undefined.empty));

      test
        "mockImplementation - sets implementation to use for subsequent \
         invocations" (fun _ ->
          let mockFn = JestJs.inferred_fn () in
          let fn = MockJs.fn mockFn in
          ...
          mockFn
          |> MockJs.mockImplementation (fun [@u] a ->
                 Js.Undefined.return (string_of_int a))
          |> ignore;

          expect (before, call fn 18, call fn 24)
          |> toEqual
               ( Js.Undefined.empty,
      ...
      test
        "mockImplementationOnce - queues implementation for one subsequent \
         invocation" (fun _ ->
          let mockFn = JestJs.inferred_fn () in
          let fn = MockJs.fn mockFn in
          ...
          let before = call fn 10 in
          mockFn
          |> MockJs.mockImplementationOnce (fun [@u] a ->
                 Js.Undefined.return (string_of_int a))
          |> ignore;
          mockFn
          |> MockJs.mockImplementationOnce (fun [@u] a ->
                 Js.Undefined.return (string_of_int (a * 2)))
          |> ignore;

          expect (before, call fn 18, call fn 24, call fn 12)
          |> toEqual
               ( Js.Undefined.empty,
                 Js.Undefined.return "18",
      ...
      test "mockReturnThis - returns `this` on subsequent invocations" (fun _ ->
          ...
          expect (before, call fn (), call fn ())
          |> toEqual
               ( Js.Undefined.empty,
                 Js.Undefined.return this,
                 Js.Undefined.return this ));

      test "mockReturnValue - returns given value on subsequent invocations"
        (fun _ ->
          let mockFn = JestJs.inferred_fn () in
          let fn = MockJs.fn mockFn in
      ...
      test
        "mockReturnValueOnce - queues implementation for one subsequent \
         invocation" (fun _ ->
          let mockFn = JestJs.inferred_fn () in
          let fn = MockJs.fn mockFn in
  ...
  *)
      test "mockClear - resets instances" (fun _ ->
          ...
          expect (before, inbetween, after)
          |> toEqual ([||], [| [%obj { n = 4 }]; [%obj { n = 7 }] |], [||])));
      ...
      test
        "mockImplementation - sets implementation to use for subsequent \
         invocations" (fun _ ->
          ...
          let before = fn 10 in
          mockFn
          |> MockJs.mockImplementation (fun a -> string_of_int (a * 2))
          |> ignore;
      ...
      test
        "mockImplementationOnce - queues implementation for one subsequent \
         invocation" (fun _ ->
          ...
          let before = fn 10 in
          mockFn
          |> MockJs.mockImplementationOnce (fun a -> string_of_int (a * 3))
          |> ignore;
          mockFn
          |> MockJs.mockImplementationOnce (fun a -> string_of_int (a * 2))
          |> ignore;

          expect (before, fn 18, fn 24, fn 12)
          |> toEqual ("10", "54", "48", "12"));
  ...
  *)
  ...
  describe "fn2" (fun _ ->
      test "calls implementation" (fun _ ->
          let mockFn = JestJs.fn2 (fun [@u] a b -> string_of_int (a + b)) in
          let fn = MockJs.fn mockFn in
  ...
  *));
  ...
  describe "MockJs.new" (fun _ ->
      ...
      test "MockJs.new2" (fun _ ->
          let mockFn =
            JestJs.fn2 [%raw "function (a, b) { this.n = a * b; }"]
          in
