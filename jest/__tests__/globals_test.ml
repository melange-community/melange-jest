...
let () =
  ...
  Skip.test "fail" (fun () -> fail "");

  test "test" (fun () -> pass);

  Skip.test "test - expect fail" (fun () -> fail "");

  testAsync "testAsync" (fun finish -> finish pass);

  Skip.testAsync "testAsync - no done" (fun _ -> ());

  Skip.testAsync "testAsync - expect fail" (fun finish -> finish (fail ""));

  testAsync "testAsync - timeout ok" ~timeout:1 (fun finish -> finish pass);

  Skip.testAsync "testAsync - timeout fail" ~timeout:1 (fun _ -> ());

  testPromise "testPromise" (fun () -> Js.Promise.resolve pass);
  ...
  describe "describe" (fun () -> test "some aspect" (fun () -> pass));
  ...
  describe "afterAllAsync" (fun () ->
      describe "without timeout" (fun () ->
          ...
          describe "phase 2" (fun () ->
              test "x is suddenly 4" (fun () ->
                  if !x == 4 then pass else fail "")));
  ...
  describe "Skip" (fun () ->
      ...
      Skip.testAsync "Skip.testAsync" (fun finish -> finish pass);
      Skip.testAsync "Skip.testAsync - timeout" ~timeout:1 (fun _ -> ());

      Skip.testPromise "Skip.testPromise" (fun () -> Js.Promise.resolve pass);
  ...
  describe "Todo" (fun () -> Todo.test "Todo.test")
