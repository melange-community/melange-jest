...
module Runner (A : Asserter) : sig
  ...
  val testPromise :
    string -> ?timeout:int -> (unit -> _ A.t Js.Promise.t) -> unit
...
val testPromise :
  string -> ?timeout:int -> (unit -> assertion Js.Promise.t) -> unit
...
module Only : sig
  ...
  val testAsync :
    string -> ?timeout:int -> ((assertion -> unit) -> unit) -> unit
...
module Expect : sig
  ...
  type 'a partial = [ 'a plainPartial | 'a invertedPartial ]

  val expect : 'a -> 'a plainPartial

  val expectFn :
    ('a -> 'b) -> 'a -> (unit -> 'b) plainPartial (* EXPERIMENTAL *)
...
module ExpectJs : sig
  ...
  val toContainProperties :
    string array -> [< < .. > Js.t partial ] -> assertion
